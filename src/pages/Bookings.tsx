...
const Bookings = () => {
  ...
  const [selectedDesktopBooking, setSelectedDesktopBooking] = useState<any>(null);
  const navigate = useNavigate();

  // Upcoming booking countdown
  const eventDate = new Date('2026-08-30T08:00:00');
  const getTimeLeft = () => {
    const now = new Date();
  ...
  return (
    <div
      className="bookings-page-container"
      style={{
        position: 'fixed',
        top: 0,
        left: 0,
    ...
    >
      ...
      <div style={{ position: 'relative', marginBottom: '24px' }}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#717171" strokeWidth="2.5" style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', paddingBottom: '64px' }}>
        ...
        <div
          onClick={() => {
            const bookingData = { name: 'Sri Venkata Carters', eventDate: '30 August 2026', eventDateRaw: '2026-08-30T08:00:00', status: 'Upcoming', image: 'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&q=80&w=200&h=200', location: 'Road No. 21, Building 3B, Flat 406, Gachibowli, Hyderabad, Telangana, 500032' };
            if (window.innerWidth >= 768) {
              setSelectedDesktopBooking(bookingData);
            } else {
              navigate('/booking-detail', { state: { booking: bookingData } });
            }
          }}
        ...
        >
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center', padding: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div style={{ fontSize: '12px', fontWeight: '500', color: '#717171' }}>Event: 30 August 2026</div>
        ...
        <div
          onClick={() => {
            const bookingData = { name: 'Grand Spice Kitchen', eventDate: '10 June 2026', eventDateRaw: '2026-06-10T08:00:00', status: 'Completed', image: 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&q=80&w=200&h=200', location: 'Plot 12, Jubilee Hills, Hyderabad, Telangana, 500033' };
            if (window.innerWidth >= 768) {
              setSelectedDesktopBooking(bookingData);
            } else {
              navigate('/booking-detail', { state: { booking: bookingData } });
        ...
        <div className="bookings-detail-pane">
          {selectedDesktopBooking ? (
            <BookingDetail bookingData={selectedDesktopBooking} isDesktop={true} />
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#717171' }}>
